            ...
            <div className="pt-6 mt-6">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Switch id="hide-price" checked={!editedCourse.priceVisible} onCheckedChange={checked => updateCourseData('priceVisible', !checked)} />
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Switch id="hide-course" checked={editedCourse.hidden || false} onCheckedChange={checked => updateCourseData('hidden', checked)} />
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Switch id="enable-certificate" checked={editedCourse.hasCertificate || false} onCheckedChange={checked => updateCourseData('hasCertificate', checked)} />
                </div>
                
                {/* Certificate Upload - Only show if certificates are enabled */}
                {editedCourse.hasCertificate && <div className="ml-6 mt-2">
                    <Label htmlFor="certificate-template">Certificate Template</Label>
                    <p className="text-sm text-muted-foreground mb-2">
                      Upload a custom certificate template
                    </p>
                    <ImageUpload value={editedCourse.certificateTemplate || ""} onChange={url => updateCourseData('certificateTemplate', url)} />
                  </div>}
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Switch id="enrollment-limit" checked={editedCourse.hasEnrollmentLimit || false} onCheckedChange={checked => updateCourseData('hasEnrollmentLimit', checked)} />
                </div>
                
                {/* Enrollment Limit Input - Only show if enrollment limits are enabled */}
                {editedCourse.hasEnrollmentLimit && <div className="ml-6 mt-2">
                    <Label htmlFor="max-enrollments">Maximum Enrollments</Label>
                    <Input id="max-enrollments" type="number" min="1" value={editedCourse.maxEnrollments || ""} onChange={e => updateCourseData('maxEnrollments', parseInt(e.target.value))} placeholder="100" className="mt-1 w-24" />
                  </div>}
              </div>
            </div>
          </div>
